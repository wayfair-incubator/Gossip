...
namespace Gossip.UnitTests.Queries
{
    ...
    public class QuerySingleOrDefaultAsyncTests
    {
        public class When_multiple_results_can_be_returned
        {
            ...
            [Test]
            public async Task It_should_throw_an_InvalidOperationException()
            {
                ...
                Assert.ThrowsAsync<InvalidOperationException>(() => conn.QuerySingleOrDefaultAsync<TableTest>("SELECT Id, Name FROM tblTest WHERE Grouping='TwoRows'"));
            }
        }
        ...
        public class When_one_result_can_be_returned
        {
            ...
            public async Task It_should_return_the_record()
            {
                ...
                var result = await conn.QuerySingleOrDefaultAsync<TableTest>("SELECT Id, Name FROM tblTest WHERE Grouping='OneRow'");
        ...
        public class When_no_data_is_available_to_be_queried
        {
            ...
            public async Task It_should_return_null()
            {
                ...
                var result = await conn.QuerySingleOrDefaultAsync<TableTest>("SELECT Id, Name FROM tblTest WHERE Grouping='NoRows'");
